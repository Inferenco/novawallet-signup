...
module NovaWalletGames::chips_tests {
    ...
    fun setup_without_ack(framework: &signer, wallet: &signer, admin: &signer, user: &signer) {
        ...
        gaming_consent::init_for_test(wallet);
        chips::init_for_test(admin);
    }

    fun setup(framework: &signer, wallet: &signer, admin: &signer, user: &signer) {
        setup_without_ack(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_chip_treasury_auto_initialized(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        assert!(chips::get_chip_treasury_balance() == INITIAL_CHIP_TREASURY_BALANCE, 0);
    }

    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_claim_free_chips_once(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    fun test_claim_without_ack_fails(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup_without_ack(framework, wallet, admin, user);
        chips::claim_free_chips(user);
    }
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    #[expected_failure(abort_code = E_TERMS_NOT_ACKNOWLEDGED, location = NovaWalletGames::chips)]
    fun test_purchase_without_ack_fails(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup_without_ack(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    #[expected_failure(abort_code = E_TERMS_NOT_ACKNOWLEDGED, location = NovaWalletGames::chips)]
    fun test_claim_after_terms_bump_without_reack_fails(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    #[expected_failure(abort_code = E_TERMS_NOT_ACKNOWLEDGED, location = NovaWalletGames::chips)]
    fun test_purchase_after_terms_bump_without_reack_fails(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        configure_default_multiplier(admin, 2, 100 * OCTAS_PER_CEDRA, 1000);
        gaming_consent::set_terms(
            wallet,
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_reack_after_terms_bump_restores_access(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        configure_default_multiplier(admin, 2, 100 * OCTAS_PER_CEDRA, 1000);
        gaming_consent::set_terms(
            wallet,
    ...
    fun test_claim_free_chips_twice_fails(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_claim_after_custom_period(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_purchase_multiplier_fresh(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        configure_default_multiplier(admin, 2, 100 * OCTAS_PER_CEDRA, 1000);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_purchase_updates_treasury_balance(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_purchase_multiplier_upgrade(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        configure_default_multiplier(admin, 2, 100 * OCTAS_PER_CEDRA, 1000);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_multiplier_no_duration_extension(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        configure_default_multiplier(admin, 2, 100 * OCTAS_PER_CEDRA, 1000);
        chips::set_multiplier_price(admin, 4, 300 * OCTAS_PER_CEDRA);
    ...
    fun test_purchase_multiplier_downgrade_fails(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    fun test_purchase_invalid_multiplier_fails(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        chips::purchase_multiplier(user, 9);
    }

    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_claim_with_active_multiplier(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        chips::update_daily_free_chip_amount(admin, 100);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_claim_after_multiplier_expires(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        chips::update_daily_free_chip_amount(admin, 100);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_admin_set_multiplier_price(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        chips::set_multiplier_price(admin, 2, 100 * OCTAS_PER_CEDRA);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    #[expected_failure(abort_code = E_INVALID_MULTIPLIER, location = NovaWalletGames::chips)]
    fun test_admin_remove_multiplier_option(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_removed_option_keeps_active_multiplier(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        chips::update_daily_free_chip_amount(admin, 100);
        configure_default_multiplier(admin, 3, 100 * OCTAS_PER_CEDRA, 1000);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_admin_set_duration(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        chips::set_multiplier_price(admin, 2, 100 * OCTAS_PER_CEDRA);
    ...
    fun test_non_admin_cannot_configure(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        chips::set_multiplier_price(user, 2, 100 * OCTAS_PER_CEDRA);
    }

    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_multiplier_options_view(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_upgrade_uses_original_duration(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        configure_default_multiplier(admin, 2, 100 * OCTAS_PER_CEDRA, 1000);
        chips::set_multiplier_price(admin, 4, 300 * OCTAS_PER_CEDRA);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_multiplier_status_view(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
        configure_default_multiplier(admin, 2, 100 * OCTAS_PER_CEDRA, 1000);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    fun test_multiplier_duration_view(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
    ...
    #[test(admin = @NovaWalletGames, user = @0x123, wallet = @wallet, framework = @0x1)]
    #[expected_failure(abort_code = E_INVALID_MULTIPLIER, location = NovaWalletGames::chips)]
    fun test_get_multiplier_price(admin: &signer, user: &signer, wallet: &signer, framework: &signer) {
        setup(framework, wallet, admin, user);
