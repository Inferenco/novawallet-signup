...
#[test_only]
module NovaWalletGames::poker_hand_eval_tests {
    ...
    fun test_high_card() {
        ...
        let (hand_type, _) = poker_hand_eval::evaluate_hand(cards);
    ...
    fun test_compare_hands() {
        ...
        assert!(poker_hand_eval::compare_hands(9, 0, 8, 7) == 1, 1);
        
        // Pair of kings beats pair of queens (tiebreaker: rank)
        assert!(poker_hand_eval::compare_hands(1, 11 << 24, 1, 10 << 24) == 1, 2);
        
        // Same hand = tie
        assert!(poker_hand_eval::compare_hands(5, 100, 5, 100) == 0, 3);
        
        // Flush beats straight
        assert!(poker_hand_eval::compare_hands(5, 0, 4, 12) == 1, 4);
    }
    ...
    #[test]
    #[expected_failure(abort_code = 1, location = NovaWalletGames::poker_hand_eval)]
    fun test_wrong_card_count_6_cards() {
        // Only 6 cards - should fail
        let cards = vector[0u8, 1, 2, 3, 4, 5];
        poker_hand_eval::evaluate_hand(cards);
    }
    ...
    #[test]
    #[expected_failure(abort_code = 2, location = NovaWalletGames::poker_hand_eval)]
