...
module NovaWalletGames::poker_pot_manager {
    use std::vector;
    ...
    /// Add dead money directly to a pot without attributing it to any player's bet.
    /// Used for forced table-level adjustments like missed-blind dead money.
    public fun add_dead_money(state: &mut PotState, amount: u64, eligible_players: vector<u64>) {
        if (amount == 0 || vector::is_empty(&eligible_players)) {
            return
        };

        let pot_idx = 0u64;
        let len = vector::length(&state.pots);
        while (pot_idx < len) {
            let pot = vector::borrow_mut(&mut state.pots, pot_idx);
            if (vectors_equal(&pot.eligible, &eligible_players)) {
