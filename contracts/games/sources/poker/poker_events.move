...
module NovaWalletGames::poker_events {
    ...
    #[event]
    struct TableMetadataUpdated has drop, store {
        table_addr: address,
        owner: address,
        name: String,
        color_index: u8,
    ...
    // Opt-in reveal for folded players (audit trail)
    #[event]
    struct HoleCardsRevealed has drop, store {
        table_addr: address,
        hand_number: u64,
        timestamp: u64,
        seat_idx: u64,
        player: address,
        hole_cards: vector<u8>,      // 2 cards, 0-51 encoding
        encryption_key: vector<u8>,  // 32 bytes for third-party verification
    }
    ...
    struct HandAborted has drop, store {
        table_addr: address,
        hand_number: u64,
        reason: u8,  // 0=commit_timeout, 1=reveal_timeout, 2=emergency, 3=voted
    }
    ...
    #[event]
    struct AbortRequested has drop, store {
        table_addr: address,
        hand_number: u64,
        requester: address,
        deadline: u64,
    ...
    #[event]
    struct AbortVoteCast has drop, store {
        table_addr: address,
        hand_number: u64,
        voter: address,
        approved: bool,
    ...
    #[event]
    struct AbortRequestCancelled has drop, store {
        table_addr: address,
        hand_number: u64,
    }
    ...
    public(friend) fun emit_table_created(
        ...
        event::emit(TableCreated { 
            table_addr, owner, small_blind, big_blind, 
            min_buy_in, max_buy_in, ante, straddle_enabled,
            name, color_index,
        });
    }

    public(friend) fun emit_table_metadata_updated(
        table_addr: address,
        owner: address,
        name: String,
        color_index: u8,
    ) {
        event::emit(TableMetadataUpdated { table_addr, owner, name, color_index });
    }

    public(friend) fun emit_table_closed(table_addr: address, owner: address) {
        event::emit(TableClosed { table_addr, owner });
    }
    ...
    public(friend) fun emit_abort_requested(
        table_addr: address,
        hand_number: u64,
        requester: address,
        deadline: u64,
    ) {
        event::emit(AbortRequested { table_addr, hand_number, requester, deadline });
    }

    public(friend) fun emit_abort_vote_cast(
        table_addr: address,
        hand_number: u64,
        voter: address,
        approved: bool,
        ...
        event::emit(AbortVoteCast { table_addr, hand_number, voter, approved, current_approvals, current_vetos });
    }

    public(friend) fun emit_abort_request_cancelled(table_addr: address, hand_number: u64) {
        event::emit(AbortRequestCancelled { table_addr, hand_number });
    }

    public(friend) fun emit_hole_cards_revealed(
        table_addr: address,
        hand_number: u64,
        timestamp: u64,
        seat_idx: u64,
        ...
        event::emit(HoleCardsRevealed {
            table_addr, hand_number, timestamp, seat_idx, player, hole_cards, encryption_key
        });
    }
    ...
    public(friend) fun emit_ownership_transferred(
        ...
        event::emit(OwnershipTransferred { table_addr, old_owner, new_owner });
    }
}
